...
def update_table(Route, BMP_value, EMP_value, Year_1, Year_2, present_year_dash, future_year_dash):
    ...
    if Year_1 <= future_year_dash:
        df2[f'AADT {Year_1}'] = df2[f'AADT {present_year_dash}']*((1+(((df2[f'AADT {future_year_dash}']/df2[f'AADT {present_year_dash}'])**(1/(future_year_dash-present_year_dash)))-1))**(Year_1 - present_year_dash))
    else:
        df2[f'AADT {Year_1}'] = df2[f'AADT {future_year_dash}']*((1+(((df2[f'AADT {future_year_dash}']/df2[f'AADT {present_year_dash}'])**(1/(future_year_dash-present_year_dash)))-1))**(Year_1 - future_year_dash))
    if Year_2 <= future_year_dash:
        df2[f'AADT {Year_2}'] = df2[f'AADT {present_year_dash}']*((1+(((df2[f'AADT {future_year_dash}']/df2[f'AADT {present_year_dash}'])**(1/(future_year_dash-present_year_dash)))-1))**(Year_2 - present_year_dash))
    else:
        df2[f'AADT {Year_2}'] = df2[f'AADT {future_year_dash}']*((1+(((df2[f'AADT {future_year_dash}']/df2[f'AADT {present_year_dash}'])**(1/(future_year_dash-present_year_dash)))-1))**(Year_2 - future_year_dash))
    df2[f'AADT {Year_1}'] = df2[f'AADT {Year_1}'].astype(int)
    df2[f'AADT {Year_2}'] = df2[f'AADT {Year_2}'].astype(int)
    
    # List all the non-duplicate years and add commas to AADT values, any duplicate AADT columns will disappear
    all_years = [present_year_dash, future_year_dash, Year_1, Year_2]
    all_years = list(set(all_years))
    for year in all_years:
        df2[f'AADT {year}'] = df2[f'AADT {year}'].apply(lambda x: '{:,.0f}'.format(x) if pd.notnull(x) else '')
